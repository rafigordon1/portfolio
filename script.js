document.addEventListener('DOMContentLoaded', () => {
  ...
  // Mobile Tap Hint Logic
  const mobHint = document.querySelector('.mobile-inline-hint');
  if(mobHint) {
    const removeHint = () => {
       mobHint.style.opacity = '0';
       setTimeout(() => mobHint.style.visibility = 'hidden', 300);
       window.removeEventListener('touchstart', removeHint);
       window.removeEventListener('click', removeHint);
       if(audioCtx.state === 'suspended') audioCtx.resume();
    };
    window.addEventListener('touchstart', removeHint, {passive:true});
  ...
  const hints = document.querySelectorAll('.tap-hint, .mobile-inline-hint');
